...
from app.models import File, FileData  # ✅ Both from __init__.py
...
class FileService:
    ...
    async def process_and_save_file(
        ...
        file_path, filename = await self.save_uploaded_file(file)
         ...
         # 2. Read content HERE (file pointer might already be at end!)
        # RESET pointer first to be safe
        await file.seek(0)  # ← RESET TO BEGINNING
        content = await file.read()  # ← NOW READ

         # Debug check
        print(f"DEBUG file_service: Read {len(content)} bytes")
